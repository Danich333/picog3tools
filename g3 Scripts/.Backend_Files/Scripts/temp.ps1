...
function Play-Sound {
	do {
    	...
    	Invoke-Expression "$Command shell am start -n com.picovr.wing.videoplayer/.UnityActivity -d file:///sdcard/sounds/sound.mp4 > $null"
		...
		if ($choice -eq 'S') {
			...
			Invoke-Expression "$Command shell am force-stop com.picovr.wing.videoplayer > $null"
